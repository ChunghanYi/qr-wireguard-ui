...
func WireGuardServerInterfaces(db store.IStore) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		//BEPLUGINS ==============================================================
		{
			...
			smsg.FieldCount = "field_count:=2\n"
			smsg.KeyValue[0] = fmt.Sprintf("Addresses:=%s\n", serverInterface.Addresses[0])
			smsg.KeyValue[1] = fmt.Sprintf("ListenPort:=%d\n", serverInterface.ListenPort)
...
func GlobalSettingSubmit(db store.IStore) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		//BEPLUGINS ==============================================================
		{
			...
			smsg.KeyValue[2] = fmt.Sprintf("PersistentKeepalive:=%d\n", globalSettings.PersistentKeepalive)
